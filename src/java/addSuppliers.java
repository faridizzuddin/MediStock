...
public class addSuppliers extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        supplier.setSupplierPhNum(supplierPhNum);
        supplier.setStorageLocation(storageLocation);
        supplier.setSupplierEmail(supplierEmail);
        supplier.setSupplierCompanyName(companyName);

        int status = MedicationDB.save(supplier);
        String stat = null;
        if (status > 0) {
            stat = "Supplier saved sucessfully";

        } else {
            stat = "Sorry! unable to save record";
        }

        request.setAttribute("status", stat);
        request.getRequestDispatcher("completeAddSupplier.jsp").forward(request, response);
    }
